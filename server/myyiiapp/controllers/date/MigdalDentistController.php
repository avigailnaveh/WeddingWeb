...
class MigdalDentistController extends Controller {

    public function actionImportMigdalDentist()
    {
        echo "\nהתחלת הפונקציה...";
        ...
        $rows = (new Query())->from('_migdal_dentist')->each(200);
        echo "\n התחיל עיבוד נתונים...";
        $countrOfFound = 0;
        $counter = 0;
        foreach ($rows as $row) {
            ...
            $clinicName = trim($row['ClinicName'] ?? '');
            $mainSpecialization = 'רפואת שיניים';
            $specialization =  trim($row['MappedSpecialization'] ?? '');
            ...
            if (preg_match("/^(ד\"ר|דר'|דר)\s*/u", $fullName, $matches)) {
                // אם נמצא, מסיר מהשם ושם את זה ב-title
                $fullName = preg_replace("/^(ד\"ר|דר'|דר)\s*/u", '', $fullName);
