...
class PsychoanalysisController extends Controller {

    public function actionImportPsychoanalysis()
    {
        echo "\nהתחלת הפונקציה...";
        ...
        $rows = (new Query())->from('_psychoanalysis')->each(200);
        echo "\n התחיל עיבוד נתונים...";
          ...
          $hasDrTitle = false;
            ...
            if (isset($fullNameParts[0])) {
                if (preg_match('/^ד[״"\']?ר\.?$/u', $fullNameParts[0])) {
                    ...
                    // גם מה- firstName נוריד את הד"ר
                    $firstNameParts = explode(' ', $firstName);
                    if (isset($firstNameParts[0]) && preg_match('/^ד[״"\']?ר\.?$/u', $firstNameParts[0])) {
                        array_shift($firstNameParts);
    ...
    private function updateProfessionalDetails($professional, $row, $categories, &$careList, &$languagesList, $mainSpecialization, $careNormalMap, $city)
    {
        ...
        $unionsName = 'החברה הפיסכו אנליטית';
    ...
    private function updateProfessionalLanguages($professional, $row, &$languagesList)
    {
        $translations = [
            ...
            'Italian' => 'איטלקית',
            'Polish' => 'פולנית',
            'Norwegian Bokmal' => 'נורבגית (בוקמול)',
            'Swedish' => 'שוודית',
            'עברית' => 'עברית',
            'אנגלית' => 'אנגלית'
        ];
        ...
        foreach ($langs as $lang) {
            ...
            // תרגום אם קיים
            $translatedLang = $translations[$lang] ?? $lang;

            if (!isset($languagesList[$translatedLang])) {
                // בדיקה אם השפה כבר קיימת בבסיס נתונים
                $langId = (new Query())->select('id')->from('speaking_language')->where(['name' => $translatedLang])->scalar();
                ...
                if (!$langId) {
                    ...
                    $langId = Yii::$app->db->getLastInsertID();

                    echo "\nשפה חדשה נוספה: {$translatedLang}";
                }

                $languagesList[$translatedLang] = $langId;
            }

            $langId = $languagesList[$translatedLang];

            // קישור בין professional לשפה
