...
class IaapsytherapyController extends Controller {

    public function actionImportIaapsytherapy()
    {
        echo "\nהתחלת הפונקציה...";
        ...
        $rows = (new Query())->from('_iaapsytherapy')->each(200);
        echo "\n התחיל עיבוד נתונים...";
        ...
        foreach ($rows as $row) {
            ...
            $mainSpecialization = trim('טיפול בעזרת בעלי חיים');
            $languages = trim('');
            $phoneNumbers = trim($row['phone'] ?? ''); 
            $phoneNumbers2 = trim($row['phone2'] ?? '');
            $city = trim($row['city'] ?? '');
            $address = trim($row['address'] ?? '');
            $gender = trim($row['gender'] ?? '');
            $about = '';
            if (!empty($phoneNumbers2)) {
