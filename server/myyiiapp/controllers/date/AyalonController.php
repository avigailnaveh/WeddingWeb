...
class AyalonController extends Controller {

    public function actionImportAyalon()
{
    echo "\nהתחלת הפונקציה...";
    ...
    $rows = (new Query())->from('_ayalon')->each(200);
    echo "\n התחיל עיבוד נתונים...";
    ...
    foreach ($rows as $row) {
        ...
        $specialization = trim($row['nameSpecialization'] ?? '');
        $specialistCert = trim($row['nameSubSpecialization'] ?? '');
        $licenseFromAyalon = trim($row['license_number'] ?? '');
        $title = trim($row['title'] ?? '');
        $languages = trim($row['languages'] ?? '');
        $phoneNumbers = trim($row['phone'] ?? '');
        $phoneMobile = trim($row['MobilePhone'] ?? '');
        ...
        if (!empty($licenseFromAyalon)) {
            ...
            $foundProfessional = Professional::find()
                    ...
                    ->andWhere(['professional_insurance.insurance_id' => 4]) 
                    ->one();
