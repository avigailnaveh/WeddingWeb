...
class IdaController extends Controller {
    ...
    public function actionImportIda()
    {
        echo "\nהתחלת הפונקציה...";
        ...
        $rows = (new Query())->from('_ida')->each(200);
        echo "\n התחיל עיבוד נתונים...";
        ...
        foreach ($rows as $row) {
            ...
            $address = trim($row['clinic_address'] ?? '');
            $city = trim($row['clinic_city'] ?? '');
    ...
    private function updateProfessionalDetails($professional, $row, &$careList, &$mainCareList, &$languagesList, $mainSpecialization, $city, $specialization)
    {
        ...
        // === עיבוד תת-התמחות (EXPERTISE) ===
        if ($specializationCleaned !== '') {
            if (isset($careList[$specializationCleaned])) {
                $careId = $careList[$specializationCleaned];
                echo "\nהתאמה נמצאה בתת-התמחות: $specializationCleaned";
            } else {
                // יצירת תת-התמחות חדשה
                echo "היי מפה:   $specializationCleaned \n";
                Yii::$app->db->createCommand()->insert('expertise', [
                    'name' => $specializationCleaned,
                    'category_id' => null,
                    'sub_category_id' => null,
                ])->execute();
                $careId = Yii::$app->db->getLastInsertID();
                $careList[$specializationCleaned] = $careId;
                echo "\nתת-התמחות חדשה נוספה: $specializationCleaned (ID: $careId)";
            }
            ...
            if ($careId) {
                ...
                if (!$exists) {
                    ...
                    echo "\nקושר רופא לתת-התמחות: $specializationCleaned";
                }
            }
        }

        // === עיבוד איגוד ===
        $unionsName = 'ההסתדרות לרפואת שיניים';
        ...
        if ($mainCareId && $careId) {
            ...
            if (!$exists) {
                ...
                echo "\nקושר התמחות ראשית ($mainSpecializationCleaned) לתת-התמחות ($specializationCleaned)";
            } else {
                echo "\nקישור כבר קיים בין התמחות ראשית ($mainSpecializationCleaned) לתת-התמחות ($specializationCleaned)";
            }
    ...
    private function updateProfessionalAddress($professional, $row, $city)
    {
        ...
        $street = trim($row['clinic_address'] ?? '');
        $phones = [];
