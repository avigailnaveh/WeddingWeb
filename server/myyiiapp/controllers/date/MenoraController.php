...
class MenoraController extends Controller {

    public function actionImportMenora()
{
    echo "\nהתחלת הפונקציה...";
    ...
    $rows = (new Query())->from('_menora')->each(200);
    echo "\n התחיל עיבוד נתונים...";
    ...
    foreach ($rows as $row) {
        ...
        $fullName = trim($row['label'] ?? '');
        $firstName = trim($row['label'] ?? '');
        $specialization = trim($row['specializations'] ?? '');
        ...
        echo "\n {$phoneNumbers}";
        $address = trim($row['addressStreet'] ?? '');
        $city = trim($row['addressCity'] ?? '');
        $houseNumber = trim($row['addressNumber'] ?? '');
    ...
    private function findProfessionalByLicense($licenseFromAyalon)
    {
        ...
        $foundProfessional = Professional::find()
            ...
            ->andWhere(['professional_insurance.insurance_id' => 3]) 
            ->one();
    ...
    private function updateProfessionalDetails($professional, $row, $categories, &$expertiseList, &$languagesList, $specialization, $specialistCert)
    {
        ...
        // עדכון כתובת
        $city = trim($row['addressCity'] ?? '');
        $street = trim($row['addressStreet'] ?? '');
        $numberRaw = trim($row['addressNumber'] ?? '');
