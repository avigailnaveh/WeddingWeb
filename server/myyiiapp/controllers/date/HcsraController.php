...
class HcsraController extends Controller {

    public function actionImportHcsra()
{
    echo "\nהתחלת הפונקציה...";
    ...
    $rows = (new Query())->from('_hcsra_doctor')->each(200);
    echo "\n התחיל עיבוד נתונים...";
    ...
    foreach ($rows as $row) {
        ...
        $specialistCert = trim($row['occupations'] ?? '');
        $licenseFromAyalon = trim($row['LicenseNumber'] ?? '');
        $title = trim($row['Title'] ?? '');
        $gender = trim($row['genderLabel'] ?? '');
        $languages = trim($row['doctorLanguages'] ?? '');
        $phoneNumbers = trim($row['Phone'] ?? '');
        $address = trim($row['Street'] ?? '');
        $city = trim($row['City'] ?? '');
        $houseNumber = trim($row['HouseNumber'] ?? '');
        $insuranceId = $this->getInsuranceIdByName('הכשרה');
        ...
        if (!empty($licenseFromAyalon)) {
            ...
            $foundProfessional = $this->findProfessionalByLicense($licenseFromAyalon, $insuranceId);
    ...
    private function findProfessionalByLicense($licenseFromAyalon, $insuranceId)
    {
        ...
        $foundProfessional = Professional::find()
            ...
            ->andWhere(['professional_insurance.insurance_id' => $insuranceId]) 
            ->one();
    ...
    private function updateProfessionalDetails($professional, $row, $categories, &$expertiseList, &$languagesList, $specialization, $specialistCert)
    {
       ...
       $phones = [];
        ...
        // עדכון שפות
        $langs = explode('|', $row['doctorLanguages'] ?? ''); // שינוי מ-| ל-,
    ...
    private function getInsuranceIdByName($insuranceName)
    {
