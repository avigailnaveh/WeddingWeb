...
class FnxDentistController extends Controller {

    public function actionImportFnxDentist()
    {
        echo "\nהתחלת הפונקציה...";
        ...
        $rows = (new Query())->from('_fnx_dentist')->each(200);
        echo "\n התחיל עיבוד נתונים...";
        $countrOfFound = 0;
        $counter = 0;
        foreach ($rows as $row) {
            ...
            $specialization =  trim($row['mapped_specialization'] ?? '');
    ...
    private function updateProfessionalDetails($professional, $row, &$careList, &$mainCareList, &$languagesList, $mainSpecialization, $city, $specialization)
    {
        ...
        // === עיבוד איגוד ===
        $unionsName = 'הפניקס רפואת שיניים';
