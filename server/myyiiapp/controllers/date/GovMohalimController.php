...
class GovMohalimController extends Controller {

    public function actionImportGovMohalim()
    {
        echo "\nהתחלת הפונקציה...";
        ...
        $rows = (new Query())->from('_gov_mohalim')->each(200);
        echo "\n התחיל עיבוד נתונים...";
        ...
        foreach ($rows as $row) {
            ...
            $city = trim($row['city'] ?? '');
            $idNumber = trim($row['confirmationnumber'] ?? '');
            $lastName = trim($row['lastname'] ?? '');
            $firstName = trim($row['firstname'] ?? '');
            $phoneNumbers = trim($row['cellphone'] ?? '');
            $phone = trim($row['phone'] ?? '');
            $categories = 'מוהל';
            $mobile = $phoneNumbers;
            if ($phone !== '' && $phone !== $phoneNumbers) {
                $phoneNumbers = $phoneNumbers !== '' ? $phoneNumbers . '|' . $phone : $phone;
            }
            ...
            if (!empty($mobile)) {
                $foundProfessional = $this->findProfessionalByPhone($mobile);
            }
                
            if ($foundProfessional) {
                echo "\nנמצא מקצוען קיים עם טלפון תואם";
                $foundProfessional->full_name = $firstName . " " . $lastName;
                ...
                if((empty($foundProfessional->phone) || $foundProfessional->phone == '') && !empty($mobile)){
                    $foundProfessional->phone = $this->cleanPhone($mobile);
                }else if ((empty($foundProfessional->phone) || $foundProfessional->phone == '') && empty($mobile) && !empty($phone)){
                ...
                if (!$foundProfessional->save()) {
                    ...
                    echo "\nמקצוען עודכן בהצלחה";
                    $this->updateProfessionalDetails($foundProfessional, $row, $street, $city, $phoneNumbers, $categories,$careList ,$mainCareList);
                }
                ...
                $professional->full_name = $firstName . " " . $lastName;
                ...
                if((empty($professional->phone) || $professional->phone == '') && !empty($mobile)){
                    $professional->phone = $this->cleanPhone($mobile);
                }else if ((empty($professional->phone) || $professional->phone == '') && empty($mobile) && !empty($phone)){
                ...
                $this->updateProfessionalDetails($professional, $row, $street, $city, $phoneNumbers, $categories ,$careList ,$mainCareList);
            }
        }
    ...
    private function updateProfessionalDetails($professional, $row, $street, $city, $phoneNumbers, $categories ,&$careList ,&$mainCareList)
    {
        ...
        $this->updateProfessionalAddress($professional, $row, $street , $city, $phoneNumbers);
    ...
    private function updateProfessionalAddress($professional, $row, $street, $city, $phones)
    {
        ...
        if (!empty($street)) {
            ...
            foreach ($cities as $cityName) {
                ...
                $city = trim($city);
                if (empty($cityName) && empty($city)) continue;
