...
class IafpController extends Controller {

    public function actionImportIafp()
    {
        echo "\nהתחלת הפונקציה...";
        ...
        $rows = (new Query())->from('_iafp')->each(200);
        echo "\n התחיל עיבוד נתונים...";
        ...
        foreach ($rows as $row) {
            ...
            if ($foundProfessional) {
                ...
                if (!$foundProfessional->save()) {
                    ...
                    echo "\nמקצוען עודכן בהצלחה";
                    $isNotNew = true;
                    $this->updateProfessionalDetails($foundProfessional, $row, $careList,$mainCareList, $languagesList, $mainSpecialization, $city,$isNotNew);
                }
                ...
                $isNotNew = false;
                $this->updateProfessionalDetails($professional, $row, $careList,$mainCareList, $languagesList, $mainSpecialization, $city ,$isNotNew);
            }
        }
    ...
    private function updateProfessionalDetails($professional, $row, &$careList,&$mainCareList, &$languagesList, $mainSpecialization, $city ,$isNotNew)
    {
       ...
       $cleaned = trim($mainSpecialization);
        ...
        $unionsName = 'האיגוד הישראלי לפסיכודרמה';
        ...
        $this->updateProfessionalAddress($professional, $row, $city ,$isNotNew);
    ...
    private function updateProfessionalAddress($professional, $row, $city ,$isNotNew)
    {
        ...
        if (!empty($city)) {
            ...
            foreach ($cities as $cityName) {
                ...
                $address = new ProfessionalAddress([
                    'professional_id' => $professional->id,
                    'city' => $cityName,
                    'type' => 'האיגוד הישראלי לפסיכודרמה',
                ]);
