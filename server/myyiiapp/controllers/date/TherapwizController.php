...
class TherapwizController extends Controller {

    public function actionImportTherapwiz()
    {
        echo "\nהתחלת הפונקציה...";
        ...
        $rows = (new Query())->from('_therapwiz')->each(200);
        echo "\n התחיל עיבוד נתונים...";
        ...
        foreach ($rows as $row) {
            ...
            $normal_care = [
                "בליעה" => "הפרעות בליעה",
                "התאמת מכשירי שמיעה" => "בדיקות שמיעה והתאמת מכשירי שמיעה",
                "שטף דיבור (גמגום/דיבור חטוף)" => "גמגום והפרעות בשטף הדיבור - מבוגרים",
                "תקשורת (ASD)" => "אוטיזם (הפרעות תקשורת) מבוגרים",
                "תקשורת תומכת וחליפית (תת\"ח)" => "תקשורת תומכת וחליפית",
            ];
            $fullName = trim($row['title'] ?? '');
            $fullName = trim(str_replace(['קלינאית תקשורת -','- קלינאית תקשורת', 'קלינאית תקשורת' , ', קלינאית תקשורת'], '', $fullName));
            $mainSpecialization = trim($row['treats_names'] ?? '');

            if ($mainSpecialization) {
                $parts = array_map('trim', explode('|', $mainSpecialization));

                $mappedParts = array_map(function($part) use ($normal_care) {
                    return $normal_care[$part] ?? $part;
                }, $parts);

                $mainSpecialization = implode('|', $mappedParts);
            }

            $languages = trim($row['languages_names'] ?? '');
            $city = trim($row['address'] ?? '');
            $about = trim($row['short_description'] ?? '');
            $title = '';
            $categories = '';
            ...
            if ($foundProfessional) {
                ...
                if (!$foundProfessional->save()) {
                    ...
                    echo "\nמקצוען עודכן בהצלחה";
                    $this->updateProfessionalDetails($foundProfessional, $row, $categories, $careList, $languagesList, $mainSpecialization, $city, $phoneNumbers);
                }
                ...
                $this->updateProfessionalDetails($professional, $row, $categories, $careList, $languagesList, $mainSpecialization, $city, $phoneNumbers);
            }
        }
    ...
    private function updateProfessionalDetails($professional, $row, $categories, &$careList, &$languagesList, $mainSpecialization, $city, $phoneNumbers)
    {
