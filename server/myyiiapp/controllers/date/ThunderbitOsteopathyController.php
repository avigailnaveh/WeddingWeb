...
class ThunderbitOsteopathyController extends Controller {

    public function actionImportThunderbitOsteopathy()
    {
        echo "\nהתחלת הפונקציה...";
        ...
        $rows = (new Query())->from('_thunderbit_osteopathy')->each(200);
        echo "\n התחיל עיבוד נתונים...";
        ...
        foreach ($rows as $row) {
            ...
            $phoneNumbers = trim($row['Phone number'] ?? '');
            $categories = trim($row['Type'] ?? '');
            $gender_limit = trim($row['gender_limit'] ?? '');
            $gender_limit = trim($gender_limit);
            $gender_limit = mb_strtolower($gender_limit);
            $gender_limit = str_replace('  ', ' ', $gender_limit);
            ...
            if ($foundProfessional) {
                ...
                if (!$foundProfessional->save()) {
                    ...
                    echo "\nמקצוען עודכן בהצלחה";
                    $this->updateProfessionalDetails($foundProfessional, $row, $street, $phoneNumbers, $categories,$careList ,$mainCareList ,$expertiseList ,$mainSpecializationList);
                }
                ...
                if(empty($professional->phone) || $professional->phone == ''){
                ...
                $this->updateProfessionalDetails($professional, $row, $street, $phoneNumbers, $categories ,$careList ,$mainCareList ,$expertiseList ,$mainSpecializationList);
            }
        }
    ...
    private function updateProfessionalDetails($professional, $row, $street, $phoneNumbers, $categories ,&$careList ,&$mainCareList ,&$expertiseList ,&$mainSpecializationList)
    {
        ...
        if (!empty($categories)) {
            ...
            foreach ($specializations as $spec) {
                ...
                $mainCareId = null;
                if($cleaned == 'מרכז טיפול בהריון'){
                    $cleaned = 'הריון';
                }

                if($cleaned == 'פיזותרפיה' || $cleaned == 'פסיכיאטריה' || $cleaned == 'רצפת האגן' || $cleaned == 'פיזותרפיה הוליסטית'){
                    ...
                    $mainCareName = preg_replace('/\s+(D\.O|N\.D)$/u', '', $cleaned);
                    $mainCareName = trim($mainCareName);
                    if($mainCareName != "פוריות" && $mainCareName != "קוסמטיקאיות" && $mainCareName != "אימון" && $mainCareName != "הדרכת הורים" && $mainCareName != "הפרעות קשב וריכוז" && $mainCareName != "פסיכותרפיה" && $mainCareName != "אוסטאופתיה" && $mainCareName != "הומאופתיה" && $mainCareName != "נטורופתיה" && $mainCareName != "ריפוי בעיסוק" && $mainCareName != "ריפוי בעיסוק ילדים" && $mainCareName != "הריון"){
                        $mainCareName = 'רפואה משלימה';
                    }
                    if (isset($mainCareList[$mainCareName])) {
                        $mainCareId = $mainCareList[$mainCareName];
                        echo "\nהתאמה נמצאה ב-name: $mainCareName";
                    }
        ...
        $unionsName = 'ThunderbitOsteopathy';
        ...
        $this->updateProfessionalAddress($professional, $row, $street , $phoneNumbers);
