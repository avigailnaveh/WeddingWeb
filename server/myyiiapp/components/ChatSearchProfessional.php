...
class ChatSearchProfessional
{
    ...
    protected $isKids;
    protected $limitSecoundQ;
    protected $localityIds; // Array of locality IDs (when restrictToArea=true, contains the locality ID found by city_name)
    protected $districtName; // District name for extending search when few results

    public function __construct(?Member $member, $page, $expertises, $mainSpecialization, $isKids = false, $company = null, $insurance = null, $address = null, $lat = null, $lng = null, $restrictToArea = false, $restrictToHmo = false, $restrictToInsurance = false, $locationType = "city")
    {
        $this->member = $member;
        ...
        if (!$insurance && $this->member) {
            ...
            // Returned format: array ([0] => 6, [1] => 5)

        }
        $this->insurance = $insurance;
        $this->limit = 15;
        $this->limitSecoundQ = 10;
        $this->offset = $this->limit * ($page - 1);
        //$this->expertises = strlen($expertises) > 1 ? explode(',', $expertises) : [];      
        ...
        Yii::info("address: " . print_r($address, 1));
        Yii::info("this address: " . print_r($this->address, 1));
        Yii::info("same address: " . print_r($address === $this->address, 1));
        
        // If restrictToArea is true, address is city_name from Localities
        // Find the locality ID to use in ProfessionalLocalities search
        if ($address && $restrictToArea) {
            ...
            Yii::info("locality: " . print_r($locality, 1));
            if($locality) {
        ...
        Yii::info("localityIds: " . print_r($this->localityIds, 1));
        Yii::info("lat: " . print_r($this->lat, 1));
        Yii::info("lng: " . print_r($this->lng, 1));    
        Yii::info("restrictToArea: " . print_r($this->restrictToArea, 1));
        Yii::info("restrictToHmo: " . print_r($this->restrictToHmo, 1));
        Yii::info("restrictToInsurance: " . print_r($this->restrictToInsurance, 1));
        Yii::info("address: " . print_r($this->address, 1));
        Yii::info("locationType: " . print_r($this->locationType, 1));
        Yii::info("company: " . print_r($this->company, 1));
        Yii::info("insurance: " . print_r($this->insurance, 1));
    }
    ...
    public function getRecommendedLikeMe()
    {
        ...
        //     'pc.professional_id = r.professional_id AND pc.company_id = :cid',
        //     [':cid' => $this->company]
        // );
        ...
        //         'pi.professional_id = r.professional_id AND pi.insurance_id = :pid',
        //         [':pid' => $this->insurance]
        //     );
        // }
        if ($this->mainSpecialization) {
            $recLikeMe->innerJoin(
                ['pms' => ProfessionalMainSpecialization::tableName()],
                'pms.professional_id = r.professional_id AND pms.main_specialization_id IN (' . implode(',', $this->mainSpecialization) . ')'
            );
        }
        if (!empty($this->expertises)) {
            $recLikeMe->innerJoin(
                ['pe' => ProfessionalExpertise::tableName()],
                'pe.professional_id = r.professional_id AND pe.expertise_id IN (' . implode(',', $this->expertises) . ')'
            );
        }
        if ($this->isKids === true) {
            $recLikeMe->innerJoin(
                ['p' => Professional::tableName()],
                'p.id = r.professional_id AND p.is_pediatric = 1'
            );
        } else {
            $recLikeMe->innerJoin(
                ['p' => Professional::tableName()],
                'p.id = r.professional_id AND p.is_pediatric = 0'
            );
        }
    ...
    public function getRecommendedFriends()
    {
        ...
        return $recFriends->where(['in', 'member_id', $allFriends]) //this is my friend
            ->andWhere(['<>', 'member_id', $this->member->id])
            //->andWhere(['category_id' => $this->category->id])//and the recommendation is of the category
            ->groupBy(['r.professional_id'])
            //  ->limit($this->limit)
            //  ->offset($this->offset)
            ->orderBy('count DESC')
    ...
    public function getRecommendedColleague()
    {
        ...
        //     );
        // }
        if (!empty($this->mainSpecialization)) {
    ...
    public function getAllRecommended()
    {
        ...
        if ($this->isKids === true) {
            ...
            if (!$this->restrictToHmo && !$this->restrictToInsurance && !$this->restrictToArea) {
                $query->innerJoin(
                    ['p' => Professional::tableName()],
                    'p.id = r.professional_id AND p.is_pediatric = 0'
                );
       ...
       if($this->restrictToArea && $this->locationType == 'district'){
            $this->addRestrictiveFiltersToRecommendation($query);
       }

        $query->groupBy(['r.professional_id'])
        ...
        Yii::info("getAllRecommended - Count: " . count($results) . " | SQL: " . $sql);
    ...
    public function getRecommendedFarFromMe($professional, $myLat, $myLng)
    {
        ...
        $query = Recommendation::find()
            ...
            ->andWhere(['<>', 'cr.lat', 0])
            ->andWhere(['<>', 'cr.lng', 0])
            ->andWhere(['<>', 'cr.id', $this->member ? $this->member->id : -1])
            ->addSelect(['r.*', 'cr.lat', 'cr.lng', 'distance' => $distanceExpr])
            ->andWhere(['<', $distanceExpr, 0.3])
            ->all();
    ...
    public function getAll()
    {
        ...
        Yii::info("getAll - Recommendation counts: " . json_encode($recommendationCounts));
        ...
        Yii::info("getAll - Sorted professional IDs: " . json_encode($sortedProfessionalIds));
        ...
        Yii::info("getAll - Recommended professionals: " . json_encode($recommendedProfessionals));

        $alreadyFetchedIds = ArrayHelper::getColumn($recommendedProfessionals, 'id');
        Yii::info("getAll - Already fetched IDs: " . json_encode($alreadyFetchedIds));
        ...
        Yii::info("getAll - Additional professionals: " . json_encode($additionalProfessionals));
        // Step 5: Merge all professionals
        $allProfessionals = array_merge($recommendedProfessionals, $additionalProfessionals);

        Yii::info("getAll - All professionals: " . json_encode($allProfessionals));
    ...
    private function fetchAdditionalProfessionals(array $excludeIds)
    {
        ...
        if ($hasRestrictiveFilter) {
            Yii::info("fetchAdditionalProfessionals - Restrictive filters are active");
            // Query with restrictive filters only
            $query = $this->buildBaseQuery();
            $this->addCommonFilters($query);
            
            // Apply restrictive filters
            // If both HMO and Insurance filters are active, use OR logic (either HMO or Insurance)
            if (($this->restrictToHmo && $hasCompany) && ($this->restrictToInsurance && $hasInsurance)) {
                $this->addHmoOrInsuranceFilter($query);
            } else {
            ...
            if ($this->restrictToArea && $this->locationType !== 'district' && count($professionals) < 10 && !empty($this->districtName) && !empty($this->localityIds)) {
                ...
                // Apply restrictive filters
                if (($this->restrictToHmo && $hasCompany) && ($this->restrictToInsurance && $hasInsurance)) {
                    $this->addHmoOrInsuranceFilter($queryDistrict);
                } else {
                    if ($this->restrictToHmo && $hasCompany) {
            ...
            if ($hasCompany) {
                Yii::info("fetchAdditionalProfessionals - Company is active but not restrictive");
                ...
                if ($hasInsurance) {
                    Yii::info("fetchAdditionalProfessionals - company and Insurance filter is active");
            ...
            if ($hasInsurance) {
                Yii::info("fetchAdditionalProfessionals - Insurance is active but not restrictive");
            ...
            // Query 3: Distance-based query (if coordinates available)
             /* if ($this->lat != 0 && $this->lng != 0) { */
                Yii::info("fetchAdditionalProfessionals - Distance-based query is active");
                ...
                Yii::info("fetchAdditionalProfessionals - Distance-based query is active");
                Yii::info("fetchAdditionalProfessionals - Professionals near me: " . json_encode($professionalsNearMe));
                if ($hasInsurance) {
    ...
    /**
     * Add HMO OR Insurance filter to query
     * Returns professionals that have either the specified HMO or Insurance (or both)
     * @param \yii\db\ActiveQuery $query
     */
    private function addHmoOrInsuranceFilter($query)
    {
        $insuranceIds = $this->getInsuranceIds();
        
        // Use EXISTS subqueries with OR condition
        $query->andWhere([
            'or',
            ['exists', (new \yii\db\Query())
                ->from(['pc' => ProfessionalCompany::tableName()])
                ->where('pc.professional_id = professional.id')
                ->andWhere(['pc.company_id' => $this->company])
            ],
            ['exists', (new \yii\db\Query())
                ->from(['pi' => ProfessionalInsurance::tableName()])
                ->where('pi.professional_id = professional.id')
                ->andWhere(['pi.insurance_id' => $insuranceIds])
            ]
        ]);
    }
    ...
    /**
     * Add common filters (specialization, expertise, isKids)
     * @param \yii\db\ActiveQuery $query
     */
    private function addCommonFilters($query)
    {
        if (count($this->mainSpecialization)) {
            $query->innerJoin(
                ['pms' => ProfessionalMainSpecialization::tableName()],
                'pms.professional_id = professional.id AND pms.main_specialization_id IN (' . implode(',', $this->mainSpecialization) . ')'
            );
        }
        ...
        if (count($this->expertises)) {
            $query->innerJoin(
                ['pe' => ProfessionalExpertise::tableName()],
                'pe.professional_id = professional.id AND pe.expertise_id IN (' . implode(',', $this->expertises) . ')'
            );
        }

        if ($this->isKids === true) {
            $query->andWhere(['professional.is_pediatric' => 1]);
        } elseif (!$this->restrictToHmo && !$this->restrictToInsurance && !$this->restrictToArea) {
    ...
    private function getCategoryArray()
    {
        ...
        if(is_array($this->expertises) && count($this->expertises) > 0) {
            $categoryModel = Expertise::findOne(['id' => $this->expertises[0]]);
            if ($categoryModel) {
                $categoryArray = $categoryModel->toArray();
                $type = 'expertises';
            }
        }
        if(is_array($this->mainSpecialization) && count($this->mainSpecialization) > 0) {
            $categoryModel = MainSpecialization::findOne(['id' => $this->mainSpecialization[0]]);
            Yii::info("categoryModel on getCategoryArray: " . print_r($categoryModel, 1));
            if ($categoryModel) {
                $categoryArray = $categoryModel->toArray();
                Yii::info("categoryArray on getCategoryArray: " . print_r($categoryArray, 1));
    ...
    {
        $categoryId = (is_array($this->mainSpecialization) && count($this->mainSpecialization) > 0) ? $this->mainSpecialization[0] : ((is_array($this->expertises) && count($this->expertises) > 0) ? $this->expertises[0] : null);
        $categoryType = (is_array($this->mainSpecialization) && count($this->mainSpecialization) > 0) ? CategoryFaq::TYPE_MAIN_SPECIALIZATION : ((is_array($this->expertises) && count($this->expertises) > 0) ? CategoryFaq::TYPE_EXPERTISE : null);
    ...
    public function getUsedParams()
    {
        ...
        Yii::info("usedParams on ChatSearchProfessional: " . print_r($usedParams, 1));
        return $usedParams;
    }
}
